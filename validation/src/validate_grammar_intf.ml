...
module type Validate_grammar = sig
  ...
  (** Validates a grammar.
      ...
      First, calls [show_grammar] unless the user passes [~show_grammar:false].

      Then, checks that [t_sexp_grammar] accepts all sexps generated by [sexp_of_t].
      ...
      failures. *)
  val validate_grammar
    :  ?test_count:int
    -> ?show_grammar:bool (** default is [true] *)
    -> (module S)
    -> unit Or_error.t
