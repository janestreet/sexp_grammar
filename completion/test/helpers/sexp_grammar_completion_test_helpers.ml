...
module Backtesting = struct
  ...
  let batch_command grammar =
    Command.async
      ...
      (let%map_open.Command () = return ()
       ...
       fun () ->
         let check = Staged.unstage (Spot_check.batchwise_check_every_position grammar) in
    ...
    Command.async
      ...
      (let%map_open.Command () = return ()
       ...
       fun () ->
         ...
         (match verbose with
          | true -> Spot_check.check_every_position grammar s
          | false ->
            let check =
              Staged.unstage (Spot_check.batchwise_check_every_position grammar)
            in
            check s ~on_error:(fun spot_check s ~pos ->
              print_endline (mark_positions s ~pos:[ pos ]);
              print_s [%sexp (spot_check : Spot_check.t)]));
         return ())
      ~behave_nicely_in_pipeline:false
  ;;
  ...
  let command grammar ~name =
    Command.group
      ~summary:
        [%string "commands for backtesting %{name} sexp grammar against real input"]
      [ "batch", batch_command grammar; "single", single_command grammar ]
  ;;
end

let backtesting_command = Backtesting.command
